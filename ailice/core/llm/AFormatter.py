...
class AFormatterVicuna:
    ...
    def __call__(self, prompt0, conversations, encode=True, assistTag=True):
        ...
        roleMap = {
            "USER": "USER",
            "ASSISTANT": "ASSISTANT",
            "SYSTEM": "SYSTEM" if not self.systemAsUser else "USER",
        }
        ret = (
            prompt0
            + "\n"
            + "".join(
                [
                    roleMap[c["role"]] + ": " + c["msg"] + sep[roleMap[c["role"]]]
                    for c in conversations
                ]
            )
            + (" ASSISTANT:" if assistTag else "")
        )
        # print("prompt: ", ret)
...
class AFormatterLLAMA2:
    ...
    def __call__(self, prompt0, conversations, encode=True, assistTag=True):
        ...
        conv = [
            {"role": roleMap[c["role"]], "msg": c["msg"]}
            for c in copy.deepcopy(conversations)
        ]

        conv[0]["msg"] = (
            (B_SYS + prompt0 + E_SYS + conv[0]["msg"])
            if self.systemAsUser or ("SYSTEM" != conv[0]["role"])
            else (prompt0 + conv[0]["msg"])
        )
        conv = [
            {"role": c["role"], "msg": B_SYS + c["msg"] + E_SYS}
            if "SYSTEM" == c["role"]
            else c
            for c in conv
        ]
        ...
        if encode:
            ...
            ret = sum(
                [
                    ...
                    for prompt, answer in zip(conv[0::2], conv[1::2])
                ],
                [],
            )
            if assistTag and (1 == (len(conv) % 2)):
                ...
                ret += self.tokenizer.encode(
                    f"{B_INST} {conv[-1]['msg'].strip()} {E_INST}"
                )
...
class AFormatterChatML:
    ...
    def __call__(self, prompt0, conversations, encode=True, assistTag=True):
        ret = (
            f"{self.START}system\n{prompt0}\n{self.END}\n"
            + "".join([self.BuildMsg(c["role"], c["msg"]) for c in conversations])
            + (f"{self.START}assistant\n" if assistTag else "")
        )
        # print("prompt: ", ret)
        return self.tokenizer.encode(ret) if encode else ret
...
class AFormatterAMAZON:
    ...
    def __call__(self, prompt0, conversations, encode=True, assistTag=True):
        ret = (
            f"{self.left['SYSTEM']}{prompt0}{self.right['SYSTEM']}"
            + "".join([self.BuildMsg(c["role"], c["msg"]) for c in conversations])
            + (f"<|assistant|>" if assistTag else "")
        )
        # print("prompt: ", ret)
        return self.tokenizer.encode(ret) if encode else ret
...
class AFormatterOpenChat:
    ...
    def __call__(self, prompt0, conversations, encode=True, assistTag=True):
        ret = (
            f"{prompt0}{self.right['SYSTEM']}"
            + "".join([self.BuildMsg(c["role"], c["msg"]) for c in conversations])
            + (f"{self.left['ASSISTANT']}" if assistTag else "")
        )
        # print("prompt: ", ret)
        return self.tokenizer.encode(ret) if encode else ret
...
class AFormatterGPT:
    ...
    def __call__(self, prompt0, conversations, encode=True, assistTag=True):
        ...
        ret = [{"role": "system", "content": prompt0}] + [
            {"role": roleMap[c["role"]], "content": c["msg"]} for c in conversations
        ]
        # print("prompt: ", ret)
        return ret
...
class AFormatterYka(AFormatterGPT):
    def __init__(self, tokenizer=None, systemAsUser=False):
        super().__init__(tokenizer, systemAsUser)
